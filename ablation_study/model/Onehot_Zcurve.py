...
class RNASeqClassifier(nn.Module):
    def __init__(self, num_layers=2, num_heads=4, drop_out=0.2, in_channels1=4, in_channels2=3):
        ...
        self.fc_layers = self._make_fc_layers(2 * conv_output_channels[-1], [128, 64, 32, 16, 8, 4, 2], drop_out)
    ...
    def forward(self, x1, x2, _):
        ...
        # Rearrange dimensions of x1, x2
        x1, x2 = x1.permute(0, 2, 1), x2.permute(0, 2, 1)

        identity1, identity2 = x1, x2

        # Pass x1, x2 through attention layers
        ...
        # Average pooling for x1, x2
        x1 = torch.mean(x1, dim=1)  
        x2 = torch.mean(x2, dim=1)  

        # Concatenate x1, x2 along the second dimension
