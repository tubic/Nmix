...
class RNASeqClassifier(nn.Module):
    def __init__(self, num_layers=2, num_heads=4, drop_out=0.2, in_channels1=4, in_channels2=3):
        ...
        # 1D convolution layers for x1 and x2
        ...
        self.fc_layers = self._make_fc_layers(2 * conv_output_channels[-1] + 16, [128, 64, 32, 16, 8], drop_out)

    def forward(self, x1, x2, x3):
        # Apply 1D CNN to x1 and x2
        ...
        # Prepare x1, x2, x3 for attention layers
    ...
    def _apply_conv_layers(self, layers, x):
        """Apply convolution layers to input x"""
        for layer in layers:
            x = layer(x)
        return x
    ...
    def _apply_fc_layers(self, layers, x):
        """Apply fully connected layers to input x"""
        for layer in layers:
            x = layer(x)
        return x
