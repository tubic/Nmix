...
class RNASeqClassifier(nn.Module):
    def __init__(self, num_layers=2, num_heads=4, drop_out=0.2, in_channels1=4, in_channels2=3):
        ...
        self.fc_layers = self._make_fc_layers(conv_output_channels[-1] + 16, [64, 32, 16, 8, 4, 2], drop_out)

    def forward(self, x1, x2, x3):
        # Pass x1 through 1D CNN
        ...
        # Rearrange dimensions of x2, x3 
        x2, x3 = x2.permute(0, 2, 1), x3.permute(0, 2, 1)

        identity2, identity3 = x2, x3

        # Pass x2, x3 through attention layers
        ...
        # Average pooling for x2, x3
        x2 = torch.mean(x2, dim=1) 
        x3 = torch.mean(x3, dim=1)  

        # Concatenate x2, x3 along the second dimension
